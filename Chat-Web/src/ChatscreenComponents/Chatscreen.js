...
function Chatscreen() {
    ...
    var handleSendMessage = async () => {
        ...
        await fetch('http://localhost:5000/api/'
            + loggedUser.username + '/Contacts/' + currentContactChat + '/Messages', {
            method: 'POST',
            headers: {
        ...
        var currentContactMessages = await fetch('http://localhost:5000/api/'
            + loggedUser.username + '/Contacts/' + currentContactChat, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
            }
        })
        setCurrentContactChat(await currentContactMessages.json());
    }
